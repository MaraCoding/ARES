...
//EXAMPLE:  timer_init(3, 1000, .5, 64);//pin 2
...
void lidarConfig(void)
{
  TCCR1A=0;
  TCCR1B=0;
  TCNT1=0;
  OCR1A = 62500;                       //Top count is 16M/(64*4), where 4 is 4hz (250ms sample rate) and 54 is the pre-scale value
...
void rpmConfig(void)
{
  ...
  TCCR4B=0;
  TCNT4=0;
  OCR4A = 62500;                       //Top count is 16M/(64*4), where 4 is 4hz (250ms sample rate) and 54 is the pre-scale value
  TIMSK4 |= (1<<OCIE4A);            //enable timer mask interrupts
  TCCR4A |= (1<<COM4A1);            //Clear on compare match mode
  TCCR4B |= (1<<WGM42);             //CTC mode FOR TIMER 1
  TCCR4B |= (1<<CS41) | (1<<CS40);  //Prescale value 64 
}
